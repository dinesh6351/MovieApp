...
import com.example.networkduo.data.network.Status
...
class MovieListFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.movies.observe(viewLifecycleOwner, Observer {
            ...
            //            movie list empty or not
//            if empty database is empty we fetch from the network fetch from the database
//            store all the data in database
//            pushes all live data in database fetch again in database
            if(it.isEmpty()){
...
private fun showErrorMessage(errorCode: ErrorCode?,message: String?){
    when(errorCode){
        ...
        ErrorCode.UNKNOW_ERROR ->status_error.text =getString(R.string.error_unknow,message)
    }
}
